...
.then(() => {// format data to fit articles table: 
  ...
  const formattedArticles = articleData.map((article) => {
    ...
    console.log(timeConvertedArticle)

    return[timeConvertedArticle.title, timeConvertedArticle.topic, timeConvertedArticle.author, timeConvertedArticle.body, timeConvertedArticle.created_at, timeConvertedArticle.votes, timeConvertedArticle.article_img_url]
  });
  const insertArticleQuery = format(`INSERT INTO articles(title, topic, author, body, created_at, votes, article_img_url) VALUES %L;`, formattedArticles);
  return db.query(insertArticleQuery)
})
// .then(() => {// query articles table for article_id and title to add to comments data
// return db.query(`SELECT article_id, title FROM articles`)
// })
// .then(() =>{// format comments data: body, article_id, author, votes, created_at, votes, article_img_url
//   const formattedComments = commentData.map((comment) => {

//     const timeConvertedComment = convertTimestampToDate(comment);
//     return[timeConvertedComment.body, timeConvertedComment.author, timeConvertedComment.votes, timeConvertedComment.created_at]
//   });
//   const insertCommentQuery = format(`INSERT INTO comments(body, author, votes, created_at) VALUES %L;`, formattedComments);
